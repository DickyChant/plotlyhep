...
def exp_text(fig: go.Figure, exp: str = "", text: str = "", *, loc: int = 0,
             lumi: str | None = None, fontsize: float | None = None,
             xref="paper", yref="paper") -> go.Figure:
    ...
    def A(txt, x, y, xa, ya, size, bold=False, italic=False, xs=0, ys=0):
        ...
        if bold: t = f"<b>{t}</b>"
        if italic: t = f"<i>{t}</i>"
        ann.append(dict(text=t, x=x, y=y, xref=xref, yref=yref, xanchor=xa, yanchor=ya,
                        showarrow=False, font=dict(size=size, family=fam), xshift=xs, yshift=ys, align="left"))
    exp_w = 0.72 * exp_px * len(exp)              # crude advance for appending text to the right
    if loc == 0:
        A(exp, 0, 1, "left", "bottom", exp_px, bold=True, ys=2)
        if text: A(text, 0, 1, "left", "bottom", txt_px, italic=True, xs=exp_w + gap, ys=2)
        if lumi: A(lumi, 1, 1, "right", "bottom", lumi_px, ys=2)
    elif loc == 1:
        A(exp, 0, 1, "left", "top", exp_px, bold=True, xs=pad, ys=-pad)
        if text: A(text, 0, 1, "left", "top", txt_px, italic=True, xs=pad + exp_w + gap, ys=-pad - (exp_px - txt_px) * 0.8)
        if lumi: A(lumi, 1, 1, "right", "bottom", lumi_px, ys=2)
    elif loc == 2:
        A(exp, 0, 1, "left", "top", exp_px, bold=True, xs=pad, ys=-pad)
        if text: A(text, 0, 1, "left", "top", txt_px, italic=True, xs=pad, ys=-pad - exp_px * 1.15)
        if lumi: A(lumi, 1, 1, "right", "bottom", lumi_px, ys=2)
    elif loc == 3:
        A(exp, 0, 1, "left", "bottom", exp_px, bold=True, ys=2)
        if text: A(text, 0, 1, "left", "top", txt_px, italic=True, xs=pad, ys=-pad)
        if lumi: A(lumi, 1, 1, "right", "bottom", lumi_px, ys=2)
    elif loc == 4:
        A(exp, 0, 1, "left", "top", exp_px, bold=True, xs=pad, ys=-pad)
        if text: A(text, 0, 1, "left", "top", txt_px, italic=True, xs=pad + exp_w + gap, ys=-pad - (exp_px - txt_px) * 0.8)
        if lumi: A(lumi, 0, 1, "left", "top", lumi_px, xs=pad, ys=-pad - exp_px * 1.15)
    else:
...
def exp_label(fig, exp="", text="", *, loc=0, data=False, year=None, lumi=None,
              lumi_format="{0}", com="13", rlabel=None, fontsize=None):
    if rlabel is None:
...
class _Exp:
    ...
    def label(self, fig, text="", **kw): return exp_label(fig, self.name, text, **kw)
    def text(self, fig, text="", **kw): return exp_text(fig, self.name, text, **kw)
...
def set_xlabel(fig: go.Figure, text: str, *, size_px: float | None = None) -> go.Figure:
    ...
    fig.add_annotation(text=text, xref="paper", yref="paper", x=1, y=0, xanchor="right", yanchor="top",
                       showarrow=False, yshift=-(pad + tick_px + pt2px(4)),
...
def set_ylabel(fig: go.Figure, text: str, *, size_px: float | None = None, ticklabel_chars: int = 4) -> go.Figure:
    ...
    fig.add_annotation(text=text, xref="paper", yref="paper", x=0, y=1, xanchor="center", yanchor="top",
