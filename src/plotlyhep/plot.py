...
def histplot(
    ...
    **kw,
    ...
    if stack:
        ...
        for h in Hs:
            acc = acc + h
            stacked.append(acc.copy())
        Hs = stacked
    if yerr is True:
        errs: list[np.ndarray | None] = [np.sqrt(np.abs(h)) for h in Hs]
    elif yerr is None or yerr is False:
        errs = [None] * len(Hs)
    else:
        errs = [np.asarray(y) for y in (yerr if isinstance(yerr, (list, tuple)) else [yerr])]
    traces = []
    ...
    for h, lab, col, err in zip(Hs, labels, colors, errs):
        ...
        if histtype in ("step", "fill"):
            ...
            t = go.Scatter(
                x=x, y=y, mode="lines", line=dict(shape="hv", width=lw, color=col), name=lab or "", showlegend=show, hoverinfo="skip", **kw
            )
            if histtype == "fill":
            ...
            traces.append(
                go.Scatter(
                    ...
                    mode="markers",
                    name=lab or "",
                    marker=dict(size=0.1, color=col),
                    showlegend=False,
            ...
            traces.append(
                go.Scatter(
                    ...
                    error_y=dict(type="data", array=err, thickness=pt2px(1), width=0, color=col) if err is not None else None,
                    **hover,
                    **kw,
                )
            )
...
def hist2dplot(fig: go.Figure, H, xbins, ybins, *, colorscale="Viridis", cbar: bool = True, **kw):
    ...
    t = go.Heatmap(
        x=xc, y=yc, z=H.T, colorscale=colorscale, showscale=cbar, xgap=0, ygap=0, colorbar=dict(thickness=pt2px(12), outlinewidth=0), **kw
    )
    fig.add_trace(t)
    return t
