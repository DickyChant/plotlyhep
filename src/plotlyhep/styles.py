...
_SNAPSHOT = {
    "CMS": {
        "font.sans-serif": ["TeX Gyre Heros", "Helvetica", "Arial"], "font.size": 26,
        "axes.labelsize": "medium", "axes.linewidth": 2,
        "axes.prop_cycle": ["#5790fc", "#f89c20", "#e42536", "#964a8b", "#9c9ca1", "#7a21dd"],
        "xtick.major.size": 12, "xtick.minor.size": 6, "ytick.major.size": 12, "ytick.minor.size": 6,
        "xtick.major.pad": 6, "xtick.labelsize": "small", "ytick.labelsize": "small",
        "legend.fontsize": "small", "figure.figsize": (10.0, 10.0),
    },
    "ATLAS": {
        "font.sans-serif": ["TeX Gyre Heros", "Helvetica", "Arial"], "font.size": 14,
        "axes.labelsize": "x-large", "axes.linewidth": 1, "lines.linewidth": 2,
        "axes.prop_cycle": ["#d55e00", "#56b4e9", "#e69f00", "#f0e442", "#009e73", "#cc79a7", "#0072b2"],
        "xtick.major.size": 5, "xtick.minor.size": 3, "ytick.major.size": 14, "ytick.minor.size": 7,
        "xtick.labelsize": "large", "ytick.labelsize": "large",
        "legend.fontsize": "medium", "figure.figsize": (8.0, 6.0),
    },
}
...
def template(exp: str = "CMS") -> go.layout.Template:
    ...
    axis = lambda major, minor: dict(
        showline=True, linecolor="black", linewidth=lw, mirror="allticks",
        ticks="inside", ticklen=pt2px(major), tickwidth=tick_w, tickcolor="black",
        minor=dict(ticks="inside", ticklen=pt2px(minor), tickwidth=mtick_w, showgrid=False),
        showgrid=False, zeroline=False, tickfont=dict(size=tick_px),
        title=dict(font=dict(size=label_px), standoff=pad_px),
        ticklabelposition="outside", automargin=False,
    )
    layout = go.Layout(
        ...
        margin=dict(l=int(SUBPLOT["left"] * W) + 2, r=int((1 - SUBPLOT["right"]) * W) + 2,
                    t=int((1 - SUBPLOT["top"]) * H) + 2, b=int(SUBPLOT["bottom"] * H) - 1, pad=0),
        ...
        legend=dict(bgcolor="rgba(0,0,0,0)", borderwidth=0, font=dict(size=leg_px),
                    x=0.98, y=0.98, xanchor="right", yanchor="top", itemsizing="constant",
