...
_DASH = {"-": "solid", "--": "dash", "-.": "dashdot", ":": "dot", "solid": "solid", "dashed": "dash", "dashdot": "dashdot", "dotted": "dot"}
_MARK = {
    "o": "circle",
    ".": "circle",
    "s": "square",
...
def from_mpl(fig, *, dpi: float | None = None) -> go.Figure:
    ...
    fam = (
        ", ".join(rc["font.sans-serif"])
        if rc["font.family"] == ["sans-serif"] or rc["font.family"] == "sans-serif"
        else ", ".join(rc["font.family"])
    )
    out = go.Figure()
    layout = dict(
        width=int(round(W)),
    ...
    for k, ax in enumerate(axes):
        xs = "x" if k == 0 else f"x{k + 1}"
        ys = "y" if k == 0 else f"y{k + 1}"
        xkey = "xaxis" if k == 0 else f"xaxis{k + 1}"
        ykey = "yaxis" if k == 0 else f"yaxis{k + 1}"
        ...
        def axis_dict(axis, lo, hi, dom, spine_lw):
            ...
            alllocs = list(axis.get_majorticklocs())
            ticktext = []
            for t in ticks:
                i = alllocs.index(t)
                ticktext.append(mathtext_to_html(labels[i]) if i < len(labels) else str(t))
            mt = axis.get_minorticklocs()
            mt = mt[(mt >= min(lo, hi)) & (mt <= max(lo, hi))]
            ...
            d = dict(
                ...
                linecolor="black",
                linewidth=spine_lw,
                mirror="allticks" if both else True,
                ticks="inside" if tdir == "in" else "outside",
                ticklen=tlen,
                tickwidth=tw,
                tickcolor="black",
                tickmode="array",
                tickvals=list(ticks),
                ticktext=ticktext,
                tickfont=dict(size=tfs),
                showgrid=False,
                zeroline=False,
                automargin=False,
                type="log" if axis.get_scale() == "log" else "linear",
                anchor=ys if axis.axis_name == "x" else xs,
            )
            if axis.get_scale() == "log":
                d["range"] = [np.log10(lo), np.log10(hi)]
            if len(mt):
                d["minor"] = dict(
                    ticks=d["ticks"],
                    ticklen=px(m1.tick1line.get_markersize()) if m1 else tlen / 2,
                    tickwidth=px(m1.tick1line.get_markeredgewidth()) if m1 else tw,
                    tickvals=list(mt),
                    showgrid=False,
                )
            return d
        ...
        for c in getattr(ax, "containers", []):
            if isinstance(c, ErrorbarContainer) and c[0] is not None:
                if not str(c.get_label()).startswith("_"):
        ...
        rank = {l: i for i, l in enumerate(leg_labels)}
        for ln in ax.lines:
            x, y = ln.get_xdata(), ln.get_ydata()
            if len(x) == 0:
                continue
            if id(ln) in relabel:
                ln.set_label(relabel[id(ln)])
            ds = ln.get_drawstyle()
            shape = {"steps-post": "hv", "steps-pre": "vh", "steps-mid": "hvh", "steps": "vh"}.get(ds, "linear")
            ls = _DASH.get(ln.get_linestyle(), "solid")
            mk = _MARK.get(str(ln.get_marker()), None)
            ...
            lab = ln.get_label()
            show = not str(lab).startswith("_")
            ey = err_for.get(id(ln))
            hov = {}
            if ey:
                hov = dict(
                    customdata=np.column_stack([np.asarray(y, float), ey[0], ey[1]]),
                    hovertemplate=(f"<b>{lab}</b><br>" if show else "")
                    + "x = %{x:g}<br>%{customdata[0]:.4g} −%{customdata[1]:.3g} / +%{customdata[2]:.3g}<extra></extra>",
                )
            out.add_trace(
                go.Scatter(
                    x=x,
                    y=y,
                    mode=mode,
                    name=lab if show else "",
                    showlegend=show,
                    xaxis=xs,
                    yaxis=ys,
                    legendrank=rank.get(lab, 1000),
                    **hov,
                    error_y=dict(type="data", symmetric=False, array=ey[1], arrayminus=ey[0], thickness=ey[2], width=0, color=col)
                    if ey
                    else None,
                    line=dict(shape=shape, width=px(ln.get_linewidth()), dash=ls, color=col),
                    marker=dict(
                        symbol=mk or "circle",
                        size=px(ln.get_markersize()) * (0.5 if ln.get_marker() == "." else 1.0),
            ...
            if isinstance(p, StepPatch):
                ...
                out.add_trace(
                    go.Scatter(
                        ...
                        fillcolor=_rgba(fc) if filled else None,
                    )
                )
                ...
                out.add_trace(
                    go.Scatter(
                        ...
                        y=v,
                        mode="markers",
                        marker=dict(size=0.1, color=_rgba(ec)),
                        showlegend=False,
                        xaxis=xs,
                        yaxis=ys,
                        name=lab if show else "",
                        **bin_hover(e, v, None, name=lab if show else None),
                    )
                )
            elif isinstance(p, Rectangle) and p.get_width() and p.get_height():
                bars_x.append(p.get_x() + p.get_width() / 2)
                bars_y.append(p.get_height())
                bars_w.append(p.get_width())
                bars_c.append(_rgba(p.get_facecolor()))
                ...
                out.add_trace(
                    go.Scatter(
                        ...
                        xaxis=xs,
                        yaxis=ys,
                        line=dict(width=px(p.get_linewidth()), color=_rgba(p.get_edgecolor())),
                        fillcolor=_rgba(p.get_facecolor()),
                    )
                )
        if bars_x:
            out.add_trace(
                go.Bar(
                    x=bars_x, y=bars_y, width=bars_w, marker=dict(color=bars_c, line=dict(width=0)), showlegend=False, xaxis=xs, yaxis=ys
                )
            )
            layout["bargap"] = 0
        for c in ax.collections:
            if isinstance(c, QuadMesh):
                ...
                z = c.get_array().reshape(coords.shape[0] - 1, coords.shape[1] - 1)
                xc = 0.5 * (coords[0, 1:, 0] + coords[0, :-1, 0])
                yc = 0.5 * (coords[1:, 0, 1] + coords[:-1, 0, 1])
                cmap = c.get_cmap()
                vmin, vmax = c.get_clim()
                scale = [[i / 10, _rgba(cmap(i / 10))] for i in range(11)]
                out.add_trace(
                    go.Heatmap(x=xc, y=yc, z=np.asarray(z), zmin=vmin, zmax=vmax, colorscale=scale, showscale=False, xaxis=xs, yaxis=ys)
                )
            elif isinstance(c, LineCollection):
                if id(c) in consumed:
                    continue
                segs = c.get_segments()
                col = _rgba(c.get_colors()[0]) if len(c.get_colors()) else "black"
                xx, yy = [], []
                for s in segs:
                    xx += [s[0][0], s[1][0], None]
                    yy += [s[0][1], s[1][1], None]
                ...
                for path in c.get_paths():
                    ...
                    out.add_trace(
                        go.Scatter(
                            ...
                            hoverinfo="skip",
                            line=dict(width=0),
                            fillcolor=_rgba(c.get_facecolor()[0]),
                        )
                    )
                ...
                out.add_trace(
                    go.Scatter(
                        ...
                        y=off[:, 1],
                        mode="markers",
                        showlegend=not c.get_label().startswith("_"),
                        name=c.get_label(),
                        xaxis=xs,
                        yaxis=ys,
                        marker=dict(
                            size=[px(np.sqrt(s)) for s in sizes] if len(sizes) > 1 else px(np.sqrt(sizes[0])),
        ...
        for im in ax.images:
            if isinstance(im, AxesImage):
                ...
                ny, nx = a.shape[:2]
                xc = np.linspace(ext[0], ext[1], nx, endpoint=False) + (ext[1] - ext[0]) / nx / 2
                ...
                out.add_trace(
                    go.Heatmap(
                        ...
                        zmin=vmin,
                        zmax=vmax,
                        colorscale=[[i / 10, _rgba(cmap(i / 10))] for i in range(11)],
                        showscale=False,
                        xaxis=xs,
                        yaxis=ys,
                    )
        ...
        for t in texts:
            ...
            if arrow is not None and arrow.get_visible():
                ...
                tf = {
                    ...
                    "figure fraction": fig.transFigure,
                    "axes points": None,
                    "figure pixels": None,
                }.get(coords, ax.transData)
                if tf is not None:
                    ...
                    pth = arrow.get_path()
                    vv = arrow.get_transform().transform(pth.vertices)
                ...
                if t.get_fontweight() in ("bold", "heavy", 700, 800, 900):
                    txt = f"<b>{txt}</b>"
                ...
                out.add_annotation(
                    ...
                    ayref="pixel",
                    showarrow=True,
                    arrowhead=2 if ("fancy" in style or "simple" in style or "curvefilled" in style) else 3,
                    arrowsize=1,
                    arrowwidth=px(arrow.get_linewidth()),
                    arrowcolor=_rgba(arrow.get_edgecolor()),
                    standoff=shrinkB,
                    xanchor="center",
                    yanchor="middle",
                    font=dict(size=px(t.get_fontsize()), color=_rgba(t.get_color())),
                )
                continue
            ...
            ay = {
                "bottom": bb.y0,
                "baseline": bb.y0,
                "center": (bb.y0 + bb.y1) / 2,
                "top": bb.y1,
                "center_baseline": (bb.y0 + bb.y1) / 2,
            }.get(va, bb.y0)
            if rot == 90:  # rotated text: anchor by the rotated box (ha applies vertically)
                ax_x = (bb.x0 + bb.x1) / 2
                ay = {"left": bb.y0, "center": (bb.y0 + bb.y1) / 2, "right": bb.y1}.get(ha, bb.y0)
                xa, ya = "center", {"left": "bottom", "center": "middle", "right": "top"}.get(ha, "bottom")
            else:
                xa = {"left": "left", "center": "center", "right": "right"}.get(ha, "left")
                ya = {"bottom": "bottom", "baseline": "bottom", "center": "middle", "top": "top", "center_baseline": "middle"}.get(
                    va, "bottom"
                )
            txt = mathtext_to_html(s)
            if t.get_fontweight() in ("bold", "heavy", 700, 800, 900):
        ...
        if lg is not None:
            ...
            layout["legend"] = dict(
                ...
                xref="paper",
                yref="paper",
                bgcolor="rgba(0,0,0,0)" if not lg.get_frame_on() else "white",
                borderwidth=0,
...
def to_mpl(pfig: go.Figure, *, dpi: float = 100.0):
    ...
    for suf, (xa, ya) in pairs.items():
        if ya is None:
            continue
        xd = xa.domain or (0.125, 0.9)
        yd = ya.domain or (0.11, 0.88)
        m = L.margin
        l = (m.l or 0) / W
        r = (m.r or 0) / W
        b = (m.b or 0) / H
        ...
        for axis, spec, setter in ((ax.xaxis, xa, ax.set_xlim), (ax.yaxis, ya, ax.set_ylim)):
            if spec.type == "log":
                axis.axes.set_xscale("log") if axis is ax.xaxis else axis.axes.set_yscale("log")
            ...
            if spec.title and spec.title.text:
                (ax.set_xlabel if axis is ax.xaxis else ax.set_ylabel)(
                    html_to_mathtext(spec.title.text), fontsize=pt(spec.title.font.size or L.font.size or 14)
                )
            d = "in" if spec.ticks == "inside" else "out"
            axis.set_tick_params(
                which="major",
                direction=d,
                length=pt(spec.ticklen or 5),
                width=pt(spec.tickwidth or 1),
            ...
            if spec.tickvals is not None:
                ...
                if spec.ticktext is not None:
                    axis.set_ticklabels([html_to_mathtext(s) for s in spec.ticktext])
        for sp in ax.spines.values():
            sp.set_linewidth(pt(xa.linewidth or 1))
        ax.set_facecolor(_mpl_color(L.plot_bgcolor) or "white")
    fig.patch.set_facecolor(_mpl_color(L.paper_bgcolor) or "white")
    for tr in pfig.data:
        suf = (tr.xaxis or "x")[1:]
        ax = axes.get(suf) or axes.get("")
        name = tr.name if getattr(tr, "showlegend", True) and tr.name else "_nolegend_"
        if tr.type == "scatter":
            x = np.asarray(tr.x, dtype=object)
            y = np.asarray(tr.y, dtype=object)
            shape = tr.line.shape or "linear"
            ds = {"hv": "steps-post", "vh": "steps-pre", "hvh": "steps-mid"}.get(shape, "default")
            col = tr.line.color or (tr.marker.color if isinstance(tr.marker.color, str) else None)
            if tr.fill in ("tozeroy", "toself"):
                xx = np.array([v for v in x if v is not None], float)
                yy = np.array([v for v in y if v is not None], float)
                ax.fill_between(
                    xx, yy, step="post" if shape == "hv" else None, color=_mpl_color(tr.fillcolor or col), linewidth=0, label=name
                )
            ...
            if "lines" in mode and (tr.line.width is None or tr.line.width > 0):
                ax.plot(
                    ...
                    y,
                    drawstyle=ds,
                    color=_mpl_color(col),
                    linewidth=pt(tr.line.width or 2),
                    linestyle={"dash": "--", "dot": ":", "dashdot": "-."}.get(tr.line.dash, "-"),
            ...
            if "markers" in mode:
                ...
                sym = {"circle": "o", "square": "s", "diamond": "D", "x": "x", "cross": "+", "triangle-up": "^"}.get(
                    tr.marker.symbol or "circle", "o"
                )
                ...
                if ey is not None:
                    ax.errorbar(
                        np.asarray(x, float),
                        np.asarray(y, float),
                        yerr=np.asarray(ey, float),
                        fmt=sym,
                        color=_mpl_color(mc),
                        markersize=pt(ms),
                        elinewidth=pt(tr.error_y.thickness or 1),
                        capsize=0,
                        label=name,
                    )
                elif ms > 0.5:
            ...
            ax.bar(
                ...
                tr.y,
                width=tr.width,
                color=[_mpl_color(c) for c in tr.marker.color]
                if isinstance(tr.marker.color, (list, tuple))
                else _mpl_color(tr.marker.color),
                linewidth=0,
                label=name,
            )
    ...
    for a in L.annotations or []:
        ...
        if a.showarrow and a.xref == "paper" and a.yref == "paper":
            ...
            ax0.annotate(
                ...
                xytext=(pt(a.ax or 0), -pt(a.ay or 0)),
                textcoords="offset points",
                ha="center",
                va="center",
                fontsize=pt(a.font.size or L.font.size or 14),
                color=_mpl_color(a.font.color) or "black",
                arrowprops=dict(
                    arrowstyle=style,
                    lw=pt(a.arrowwidth or 1),
                    color=_mpl_color(a.arrowcolor) or "black",
                    shrinkA=0,
                    shrinkB=pt(a.standoff or 0),
                ),
            )
            continue
        if a.xref == "paper" and a.yref == "paper":
            ...
            ys = (a.yshift or 0) / H
            txt = html_to_mathtext(a.text)
            weight = "bold" if "<b>" in (a.text or "") else "normal"
            style = "italic" if "<i>" in (a.text or "") else "normal"
            fig.text(
                a.x + xs,
                a.y + ys,
                txt,
                ha=a.xanchor or "center",
                va={"top": "top", "middle": "center", "bottom": "bottom"}.get(a.yanchor or "middle", "center"),
                fontsize=pt(a.font.size or L.font.size or 14),
                rotation=-(a.textangle or 0),
                fontweight=weight,
                fontstyle=style,
                color=_mpl_color(a.font.color) or "black",
                family=fam,
    ...
    if L.showlegend:
        ranks = {
            tr.name: (tr.legendrank if tr.legendrank is not None else 1000) for tr in pfig.data if tr.name and tr.showlegend is not False
        }
        for ax in axes.values():
            ...
            if h:
                ...
                ax.legend(
                    ...
                    [l[i] for i in order],
                    loc=loc,
                    frameon=bool(lg and lg.bgcolor and lg.bgcolor != "rgba(0,0,0,0)"),
                    fontsize=fs,
                )
    return fig
