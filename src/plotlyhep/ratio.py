...
def gridspec_domains(height_ratios=(3, 1), hspace=0.05, *, top=SUBPLOT["top"], bottom=SUBPLOT["bottom"]):
    ...
    r = np.asarray(height_ratios, float)
    n = len(r)
    total = top - bottom
    axes_h = total / (1 + hspace * (n - 1) / n)
    sep = hspace * axes_h / n
    h = r / r.sum() * axes_h
    doms = []
    y = top
    for hi in h:
        doms.append((round(min(1.0, max(0.0, (y - hi - bottom) / total)), 6), round(min(1.0, max(0.0, (y - bottom) / total)), 6)))
        y -= hi + sep
    return doms
...
def ratio_figure(
    ...
    *,
    ...
    ax = t.layout.xaxis.to_plotly_json()
    ay = t.layout.yaxis.to_plotly_json()
    fig.update_layout(
    ...
    fig.add_shape(
        ...
        y0=1,
        y1=1,
        line=dict(color="rgba(25,28,32,0.55)", width=pt2px(1), dash="dash"),
    )
    return fig


def ratioplot(fig: go.Figure, num, den, bins=None, *, num_w2=None, den_w2=None, band: bool = True, color="black", label=None, **kw):
    ...
    n = len(num)
    e = np.asarray(bins, float) if bins is not None else np.arange(n + 1, dtype=float)
    ctr = 0.5 * (e[1:] + e[:-1])
    ok = den > 0
    ...
    if band and den_w2 is not None:
        ...
        lo = np.concatenate([[1.0], 1 - rel, [1 - rel[-1], 1.0]])
        hi = np.concatenate([[1.0], 1 + rel, [1 + rel[-1], 1.0]])
        traces.append(
            go.Scatter(x=x, y=lo, mode="lines", line=dict(shape="hv", width=0), hoverinfo="skip", showlegend=False, xaxis="x2", yaxis="y2")
        )
        traces.append(
            go.Scatter(
                x=x,
    ...
    hover = [
        f"[{e[i]:g}, {e[i + 1]:g})<br>ratio <b>{r[i]:.3f}</b> ± {rerr[i]:.3f}<br>num {num[i]:.4g} · den {den[i]:.4g}"
        if ok[i]
        else f"[{e[i]:g}, {e[i + 1]:g})<br>empty denominator"
        for i in range(n)
    ]
    traces.append(
        go.Scatter(
            ...
            y=r,
            mode="markers",
            marker=dict(symbol="circle", size=pt2px(3.5), color=color),
            name=label or "ratio",
            showlegend=False,
            xaxis="x2",
            yaxis="y2",
            ...
            error_y=dict(type="data", array=rerr, thickness=pt2px(1), width=0, color=color),
            **kw,
        )
    )
    for t in traces:
