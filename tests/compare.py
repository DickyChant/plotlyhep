...
def case_axes_label():
    fig, ax = mpl_fig()
    ax.set_xlim(0, 200); ax.set_ylim(0, 400)
    ax.set_xlabel("m$_{jj}$ [GeV]"); ax.set_ylabel("Events")
    hep.cms.label("Preliminary", data=True, lumi=138, com=13.6, ax=ax, loc=0)
    p = php.figure("CMS"); p.update_xaxes(range=[0, 200]); p.update_yaxes(range=[0, 400])
    php.set_xlabel(p, "m<sub>jj</sub> [GeV]"); php.set_ylabel(p, "Events")
...
def case_step_hist():
    ...
    ax.set_xlim(0, 200); ax.set_ylim(0, 450); ax.legend(loc="upper right")
    ax.set_xlabel("m$_{jj}$ [GeV]"); ax.set_ylabel("Events / 5 GeV")
    hep.cms.label("Preliminary", data=True, lumi=138, com=13.6, ax=ax, loc=0)
    p = php.figure("CMS")
    ...
    p.update_xaxes(range=[0, 200]); p.update_yaxes(range=[0, 450])
    php.set_xlabel(p, "m<sub>jj</sub> [GeV]"); php.set_ylabel(p, "Events / 5 GeV")
    php.cms.label(p, "Preliminary", data=True, lumi=138, com=13.6, loc=0)
    return fig, p
...
def metrics(a: np.ndarray, b: np.ndarray) -> dict:
    d = np.abs(a - b)
    return {"mean_abs_diff": float(d.mean()), "frac_pixels_gt40": float((d > 40).mean()),
