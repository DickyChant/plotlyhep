...
def grey(img): return np.asarray(img.convert("L"), dtype=float)

def run():
    res = {}
    for name, (fn, ref, scale) in CASES.items():
        fig = fn(); W, H = fig.layout.width, fig.layout.height
        ...
        if ours.size != refi.size: ours = ours.resize(refi.size, Image.LANCZOS)
        A, B = grey(refi), grey(ours); d = np.abs(A - B)
        m = {"mean_abs_diff": float(d.mean()), "frac_pixels_gt40": float((d > 40).mean()), "ssim": float(ssim(A, B, data_range=255.0)), "size": list(refi.size)}
        res[name] = m
        ours.save(f"{OUT}/root_{name}_ours.png"); Image.fromarray((255 - d).astype(np.uint8)).save(f"{OUT}/root_{name}_diff.png")
    ...
    json.dump(res, open(f"{OUT}/root_metrics.json", "w"), indent=1); return res
