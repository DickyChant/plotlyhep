...
SRC = {
    ...
    "2012": (
        "https://opendata.cern.ch/record/12341/files/Run2012BC_DoubleMuParked_Muons.root",
...
RESONANCES = [  # name, mass GeV, width, what it is, note
    (
        ...
        0.5479,
        "1.31 keV",
        "light-quark pseudoscalar meson (uū, dd̄, ss̄)",
        "μμ branching ratio ~6×10⁻⁶ — barely visible even with 61 M events",
    ),
    ...
    (
        ...
        3.0969,
        "93 keV",
        "cc̄ 1S — the November 1974 revolution (BNL and SLAC)",
        "μμ branching ratio 6 % — the tallest narrow peak in the spectrum",
    ),
    ...
    (
        ...
        91.1876,
        "2.495 GeV",
        "the neutral weak boson — CERN 1983 (UA1, UA2)",
        "μμ branching ratio 3.4 %; the width here is the natural width, not resolution",
    ),
]
...
def reduce_2010(cache):
    url, name, desc = SRC["2010"]
    p = fetch(url, name, cache)
    import csv
    ...
    with open(p) as f:
        ...
        for row in r:
            ...
            P1.append((float(row["px1"]), float(row["py1"]), float(row["pz1"])))
            E2.append(float(row["E2"]))
            P2.append((float(row["px2"]), float(row["py2"]), float(row["pz2"])))
...
def reduce_2012(cache):
    ...
    url, name, desc = SRC["2012"]
    p = fetch(url, name, cache)
    os_c = np.zeros(len(EDGES) - 1)
    ss_c = np.zeros(len(EDGES) - 1)
    n = 0
    for a in uproot.iterate(p + ":Events", ["nMuon", "Muon_pt", "Muon_eta", "Muon_phi", "Muon_mass", "Muon_charge"], step_size="300 MB"):
        ...
        m = np.sqrt(np.maximum(ak.to_numpy(m2), 0))
        qq = ak.to_numpy(ak.prod(q, axis=1))
        os_c += np.histogram(m[qq < 0], EDGES)[0]
        ss_c += np.histogram(m[qq > 0], EDGES)[0]
...
def main(which, cache):
    ...
    out = (
        ...
        if os.path.exists(OUT)
            ...
            "edges": EDGES.tolist(),
            "resonances": [dict(zip(("name", "mass", "width", "what", "note"), r)) for r in RESONANCES],
            "datasets": {},
        }
    )
    if which == "2012root":  # ROOT-tutorial binning, for the identical plot
        ...
        d = reduce_2012(cache)
        d["edges"] = "root"
        out["root_edges"] = [0.25, 300.0, 30000]
        out["datasets"]["2012_root"] = d
        which = "2012_root"
    else:
        out["datasets"][which] = reduce_2010(cache) if which == "2010" else reduce_2012(cache)
    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    json.dump(out, open(OUT, "w"))
    d = out["datasets"][which]
    print(f"{which}: {d['n_events']} events, {sum(d['os'])} opposite-sign pairs in [0.25, 300] GeV -> {OUT}")
