    ...
    for i in range(len(ctr)):
        ...
        for key, arr in parts:
            ...
            if arr[i] > 0 or len(parts) == 1:
                lines.append(
                    f"&nbsp;&nbsp;{key}: {arr[i]:.2f} — {s['description']}<br>&nbsp;&nbsp;&nbsp;&nbsp;{s['generator']}, σ = {s['xsec_pb']:.4g} pb, DSID {s['dsid']}"
                )
...
def build(chan, sig_scale=1.0):
    ...
    for grp in GROUP_ORDER:
        G = g[grp]
        y = G["y"] * (sig_scale if grp == "Higgs" else 1.0)
        traces.append(
            go.Bar(
                x=ctr,
                y=y,
                width=width,
                name=grp + (f" × {sig_scale:g}" if grp == "Higgs" and sig_scale != 1 else ""),
                marker=dict(color=GROUP_COLOR[grp], line=dict(width=0)),
                customdata=hover_for(grp, G, total),
                hovertemplate="%{customdata}<extra></extra>",
                legendrank=GROUP_ORDER.index(grp),
            )
        )
    # data: black points with Poisson bars; hover shows observed vs expected
    obs_hover = [
        f"<b>Data</b> ({', '.join('period ' + p for p in D['data']['periods'])}, {D['lumi_fb']:g} fb⁻¹)<br>m<sub>4ℓ</sub> ∈ [{E[i]:.2f}, {E[i + 1]:.2f}) GeV<br>observed <b>{int(data[i])}</b> · expected {total[i]:.2f} (S = {g['Higgs']['y'][i]:.2f}, B = {total[i] - g['Higgs']['y'][i]:.2f})"
        for i in range(len(ctr))
    ]
    traces.append(
        go.Scatter(
...
for t in build("all"):
    fig.add_trace(t)
php.set_xlabel(fig, "m<sub>4ℓ</sub> [GeV]")
php.set_ylabel(fig, "Events / 3.75 GeV", ticklabel_chars=2)
php.atlas.label(fig, "Open Data", data=True, lumi=D["lumi_fb"], com=13, loc=1)
fig.update_layout(
    showlegend=True,
    legend=dict(x=0.98, y=0.80, xanchor="right", yanchor="top", traceorder="reversed"),
...
def restyle(trs):
    return {"y": [t.y for t in trs], "customdata": [t.customdata for t in trs], "name": [t.name for t in trs]}


# controls live in the top margin, clear of the ATLAS label and the axes
fig.update_layout(
    margin=dict(l=118, r=30, t=100, b=84),
    updatemenus=[
        dict(
            type="dropdown",
            x=0.0,
            ...
            font=dict(size=13),
            buttons=[
                dict(label=f"channel: {CHANNEL_LABEL[c]}", method="restyle", args=[restyle(chan_traces[c])])
                for c in ["all", "4e", "4mu", "2e2mu"]
            ],
        ),
        dict(
            type="buttons",
        ...
        dict(
            ...
            font=dict(size=13),
            buttons=[
                dict(label="signal × 1", method="restyle", args=[restyle(chan_traces["all"])]),
                dict(label="signal × 10", method="restyle", args=[restyle(sig10)]),
            ],
        ),
    ],
)
...
DE = np.asarray(DM["edges"])
dctr = np.sqrt(DE[1:] * DE[:-1])
dsets = [k for k in DM["datasets"] if DM["datasets"][k].get("edges") != "root"]
...
def dimuon_traces(key, sign):
    d = DM["datasets"][key]
    y = np.asarray(d[sign], float)
    yy = np.where(y > 0, y, np.nan)  # log axis: empty bins break the line
    x = np.concatenate([DE[:1], DE, DE[-1:]])
    ys = np.concatenate([[np.nan], yy, [yy[-1], np.nan]])
    hover = [
        f"m<sub>μμ</sub> ∈ [{DE[i]:.3f}, {DE[i + 1]:.3f}) GeV<br><b>{int(y[i])}</b> {'opposite' if sign == 'os' else 'same'}-sign pairs"
        + (f"<br>near the {', '.join(near_res(DE[i], DE[i + 1]))}" if near_res(DE[i], DE[i + 1]) else "")
        for i in range(len(dctr))
    ]
    step = go.Scatter(
        x=x,
        y=ys,
        mode="lines",
        line=dict(shape="hv", width=pt2px(1.2), color="#1a4480" if sign == "os" else "#9c9ca1"),
        name=f"{key} · {'opposite' if sign == 'os' else 'same'}-sign",
        hoverinfo="skip",
        showlegend=True,
    )
    carrier = go.Scatter(
        ...
        y=yy,
        mode="markers",
        marker=dict(size=8, opacity=0.02, color="#1a4480"),
        showlegend=False,
        customdata=hover,
        hovertemplate="%{customdata}<extra></extra>",
    )
...
dfig = php.figure("CMS")
for t in dimuon_traces(key0, "os"):
    dfig.add_trace(t)
...
for r in DM["resonances"]:
    ...
    if ymax < 2:
        continue  # label only peaks this dataset can show
    dfig.add_annotation(
        x=np.log10(r["mass"]),
        y=np.log10(max(ymax, 1)) + 0.18,
        xref="x",
        yref="y",
        text=r["name"],
        showarrow=True,
        ...
        arrowwidth=1,
        arrowcolor="#5c636e",
        font=dict(size=16, color="#191c20"),
        hovertext=f"<b>{r['name']}</b> — m = {r['mass']:g} GeV, Γ = {r['width']}<br>{r['what']}"
        + (f"<br>{r['note']}" if r["note"] else ""),
        hoverlabel=dict(bgcolor="white", font=dict(size=13)),
    )
dfig.update_layout(
    width=900,
    height=600,
    margin=dict(l=112, r=30, t=100, b=84),
    showlegend=True,
    ...
    xaxis=dict(
        ...
        tickvals=[0.3, 0.5, 1, 2, 3, 5, 10, 20, 30, 50, 100, 200],
        ticktext=["0.3", "0.5", "1", "2", "3", "5", "10", "20", "30", "50", "100", "200"],
        minor=dict(ticks=""),
    ),
    yaxis=dict(type="log", rangemode="normal"),
)
php.set_xlabel(dfig, "m<sub>μμ</sub> [GeV]")
php.set_ylabel(dfig, "Events / bin", ticklabel_chars=3)
php.cms.label(dfig, "Open Data", data=True, rlabel=f"{DM['datasets'][key0]['sqrt_s_TeV']} TeV", loc=0)


def dm_restyle(key, sign):
    a, b = dimuon_traces(key, sign)
    return {"y": [a.y, b.y], "customdata": [None, b.customdata], "name": [a.name, b.name], "line.color": [a.line.color, None]}
...
menus = [
    dict(
        ...
        font=dict(size=13),
        buttons=[
            dict(label="opposite-sign", method="restyle", args=[dm_restyle(key0, "os")]),
            dict(label="same-sign", method="restyle", args=[dm_restyle(key0, "ss")]),
        ],
    ),
    dict(
        type="buttons",
        ...
        font=dict(size=13),
        buttons=[
            dict(label="full", method="relayout", args=[{"xaxis.range": [np.log10(0.25), np.log10(300)]}]),
...
if len(dsets) > 1:
    menus.append(
        dict(
            ...
            buttons=[
                dict(
                    label=f"{k}: {DM['datasets'][k]['n_events'] / 1e6:.1f} M events"
                    if DM["datasets"][k]["n_events"] > 1e6
                    else f"{k}: {DM['datasets'][k]['n_events'] // 1000}k events",
                    method="restyle",
                    args=[dm_restyle(k, "os")],
                )
                for k in sorted(dsets, reverse=True)
            ],
        )
    )
...
def copy_ref(name):
    src = os.path.join(ROOT, "tests", "reference", name)
...
DR = json.load(open(os.path.join(HERE, "data", "hzz4l_root.json")))
RE = np.asarray(DR["edges"])
rctr = 0.5 * (RE[1:] + RE[:-1])
rcat = {k: np.asarray(v, float) for k, v in DR["categories"].items()}
rnom = np.asarray(DR["mc_total"]["nominal"], float)
rw2 = np.asarray(DR["mc_total"]["sumw2"], float)
rdata = np.asarray(DR["data"]["counts"], float)
rfig = php.ratio_figure("ATLAS", height_ratios=(3, 1), hspace=0.05, ratio_range=(0, 2.5))
php.histplot(
    rfig,
    [rcat["other"], rcat["zz"], rcat["higgs"]],
    RE,
    stack=True,
    histtype="fill",
    color=["#cc99ff", "#99ccff", "#990000"],
    label=["Other MC", "ZZ MC", "Higgs MC"],
)
...
rfig.update_layout(
    ...
    xaxis2=dict(range=[80, 170]),
    legend=dict(x=0.98, y=0.98, xanchor="right", yanchor="top", traceorder="reversed"),
)
php.set_ylabel(rfig, "Events / 3.75 GeV", ticklabel_chars=2)
php.set_xlabel(rfig, "m<sub>4ℓ</sub> [GeV]")
php.atlas.label(rfig, "Open Data", data=True, lumi=10, com=13, loc=1)
rfig.update_layout(
    margin=dict(l=118, r=30, t=70, b=84), hoverlabel=dict(bgcolor="white", font=dict(size=13, family="Helvetica, Arial"), align="left")
)
...
ed = os.path.join(out, "editor")
os.makedirs(os.path.join(ed, "figures"), exist_ok=True)
os.makedirs(os.path.join(ed, "templates"), exist_ok=True)
manifest = []
for keyname, title, figobj in (
    ("df102", "ROOT df102 — CMS dimuon spectrum (rebuild)", f102),
    ("df106", "ROOT df106 — ATLAS H→4ℓ (rebuild)", f106),
    ("hzz4l", "H→4ℓ in the plotlyhep ATLAS template", fig),
    ("dimuon", "dimuon spectrum in the plotlyhep CMS template", dfig),
    ("ratio", "H→4ℓ data / MC with a ratio panel", rfig),
):
    j = figobj.to_plotly_json()
    j["layout"].pop("updatemenus", None)
...
for tname, tj in (
    ("hep_cms", template_json("CMS")),
    ("hep_atlas", template_json("ATLAS")),
    ("root", php.root_template(800, 600).to_plotly_json()),
):
