...
SAMPLES = {  # key: (file, group, generator, one-line description)
    "ggH125_ZZ4lep": (
        ...
        "gluon–gluon fusion, H → ZZ* → 4ℓ, m_H = 125 GeV — the dominant production mode",
    ),
    "VBFH125_ZZ4lep": (
        "mc_344235.VBFH125_ZZ4lep.4lep.root",
        "Higgs",
        "Powheg+Pythia8",
        "vector-boson fusion, H → ZZ* → 4ℓ — two forward jets, ~8 % of the signal",
    ),
    ...
    "llll": (
        ...
        "ZZ*",
        "Sherpa 2.2.2",
        "qq̄ → ZZ* → 4ℓ — the irreducible background, same final state as the signal",
    ),
    "Zee": (
        "mc_361106.Zee.4lep.root",
        "Z+jets, tt̄",
        "Powheg+Pythia8",
        "Z → ee + jets — reducible: extra leptons from heavy flavour or misidentified jets",
    ),
...
GROUPS = {
    "Higgs": "H → ZZ* → 4ℓ, all production modes (m_H = 125 GeV)",
    "ZZ*": "ZZ* → 4ℓ, irreducible",
    "Z+jets, tt̄": "Z+jets and tt̄, reducible",
}
...
def fetch(name, cache):
    ...
    if not os.path.exists(p):
        ...
        urllib.request.urlretrieve(REC + name, p)
    return p
...
def select(t, mc):
    ...
    if mc:
        cols += ["mcWeight", "scaleFactor_PILEUP", "scaleFactor_ELE", "scaleFactor_MUON", "scaleFactor_LepTRIGGER"]
    a = t.arrays(cols)
    a = a[(a.trigE | a.trigM) & (a.lep_n == 4)]
    a = a[(ak.sum(a.lep_charge, axis=1) == 0)]
    typ = ak.sum(a.lep_type, axis=1)
    a = a[(typ == 44) | (typ == 48) | (typ == 52)]
    pt = a.lep_pt / 1000.0
    ...
    w = (
        ak.to_numpy(a.mcWeight * a.scaleFactor_PILEUP * a.scaleFactor_ELE * a.scaleFactor_MUON * a.scaleFactor_LepTRIGGER)
        if mc
        else np.ones(len(m4l))
    )
    return m4l, chan, w
...
def hist(m, w, mask=None):
    if mask is not None:
        m, w = m[mask], w[mask]
    y, _ = np.histogram(m, EDGES, weights=w)
    y2, _ = np.histogram(m, EDGES, weights=w * w)
...
def main(cache):
    ...
    ns = {}
    exec(urllib.request.urlopen(INFO, timeout=60).read().decode(), ns)
    infos = ns["infos"]
    out = {
        "source": "ATLAS Open Data, 13 TeV, 2020 release — CERN Open Data record 15005 (four-lepton skims)",
        "normalisation": "xsec [pb] × 10 fb⁻¹ / sumw, from the ATLAS outreach infofile",
        "lumi_fb": LUMI_FB,
    ...
    for key, (fn, group, gen, desc) in SAMPLES.items():
        info = infos[key]
        scale = info["xsec"] * LUMI_FB * 1000.0 / info["sumw"]
        ...
        out["samples"][key] = s
        print(f"{key:16s} {len(m):7d} selected, {sum(s['yield']):8.2f} expected", flush=True)
    md, cd = [], []
    ...
    out["data"] = {
        "periods": [p for _, p in DATA],
        "counts": np.histogram(m, EDGES)[0].tolist(),
        "selected": len(m),
