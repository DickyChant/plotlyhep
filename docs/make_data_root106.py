...
SAMPLES = {  # from df106_HiggsToFourLeptons_spec.json
    "mc_345060.ggH125_ZZ4lep": (
        ...
        "gluon–gluon fusion, H → ZZ* → 4ℓ, m_H = 125 GeV",
    ),
...
VX = np.array([5.50e3, 5.52e3, 12.54e3, 17.43e3, 22.40e3, 27.48e3, 30e3, 10000e3])
VY = np.array([0.06628, 0.06395, 0.06396, 0.03372, 0.02441, 0.01403, 0, 0])
...
def fetch(name, cache):
    ...
    if not os.path.exists(p):
        ...
        urllib.request.urlretrieve(REC + name + ".4lep.root", p)
    return p
...
def run(t, mc):
    ...
    keep = ak.sum(good, axis=1) == 4
    a, good = a[keep], good[keep]
    keep = ak.sum(a.lep_charge[good], axis=1) == 0
    a, good = a[keep], good[keep]
    st = ak.sum(a.lep_type[good], axis=1)
    keep = (st == 44) | (st == 52) | (st == 48)
    a, good = a[keep], good[keep]
    ...
    ipok = (abs(d0 / sd0) <= 5) & (abs(z0 * np.sin(theta)) <= 0.5)
    eok = (typ != 11) | ((pt >= 7000) & (abs(eta) <= 2.47))
    keep = ak.all(ipok & eok, axis=1)
    a, good = a[keep], good[keep]
    pt = a.lep_pt[good]
    keep = (pt[:, 0] > 25000) & (pt[:, 1] > 15000) & (pt[:, 2] > 10000)
    a, good = a[keep], good[keep]
...
def H(m, w):
    return np.histogram(m, EDGES, weights=w)[0]


def main(cache):
    out = {
        "source": "ATLAS Open Data 13 TeV (CERN Open Data record 15005), reduced as ROOT's df106_HiggsToFourLeptons",
        "lumi_pb": LUMI_PB,
        "edges": EDGES.tolist(),
        "samples": {},
        "categories": {},
    ...
    for name, (c, xs, sw, sc, gen, desc) in SAMPLES.items():
        ...
        out["samples"][name] = {
            ...
            "selected_raw": len(m),
            "yield": h.tolist(),
            "raw": np.histogram(m, EDGES)[0].tolist(),
        }
    ...
    for d in DATA:
        ...
        md.append(m)
    m = np.concatenate(md)
    out["data"] = {"counts": np.histogram(m, EDGES)[0].tolist(), "selected": len(m)}
